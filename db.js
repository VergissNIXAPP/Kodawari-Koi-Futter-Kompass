...
export async function exportAll(db){
  const [ponds,koi,logs,settings] = await Promise.all([
    getAll(db,"ponds"), getAll(db,"koi"), getAll(db,"logs"), getMeta(db,"settings", null)
  ]);
  return { version: 1, exportedAt: new Date().toISOString(), ponds, koi, logs, settings };
}

export async function importAll(db, data){
  if(!data || typeof data !== "object") throw new Error("Ungültige Datei.");
  const stores = ["ponds","koi","logs"];
  // wipe + import
