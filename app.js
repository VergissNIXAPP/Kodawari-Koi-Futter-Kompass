...
// Passwortschutz wurde entfernt (war nur für Test/privat gedacht)
...
const PRESET_FOODS = [
  ...
  // Kenji Koi (Kodawari Koi Shop)
  {
    ...
    temp_min_c: 10,
    temp_max_c: 30,
    tags: ["Allround","Erhalt"],
    url: "https://www.kodawari-koi.de/product-page/kenji-koi-daily-food",
    notes: "Premium‑Vollnahrung für die tägliche Fütterung ab ~10°C."
  },
  {
    id: "kenji_season",
    name: "Kenji Koi Season 5kg",
    brand: "Kenji Koi",
    category: "Übergang / leicht verdaulich",
    protein: 32.0,
    fat: 7.0,
    fiber: 2.5,
    ash: 8.0,
    ...
    temp_min_c: 8,
    temp_max_c: 20,
    tags: ["Frühjahr/Herbst","Schonfütterung"],
    url: "https://www.kodawari-koi.de/product-page/kenji-koi-season-5kg",
    notes: "Wheatgerm‑basiert, für Saisonwechsel / kühlere Tage."
  },
  {
    ...
    temp_min_c: 4,
    temp_max_c: 12,
    tags: ["Winter","Schonfütterung"],
    url: "https://www.kodawari-koi.de/product-page/kenji-koi-winter-balance-5kg",
    notes: "Sinkend & sehr leicht verdaulich (kalte Jahreszeit)."
  },
  {
    ...
    notes: "Energie‑ und proteinreiches Sommerfutter für Wachstum."
  },
  {
    ...
    notes: "Farbfutter (Astaxanthin/Spirulina) ab ~16°C."
  },
  {
    id: "kenji_color_growth_plus",
    name: "Kenji Koi Color & Growth+ 5kg",
    brand: "Kenji Koi",
    category: "Farbe + Wachstum / Performance",
    protein: 47.0,
    fat: 10.0,
    fiber: 1.2,
    ash: 9.0,
    ...
    notes: "Performance‑Futter: Wachstum + Farbentwicklung (warm)."
  },
  ...
  // Alpha (Kodawari Koi Shop)
  {
    ...
    temp_min_c: 10,
    temp_max_c: 30,
    tags: ["Allround","Erhalt","Konditionierung"],
    url: "https://www.kodawari-koi.de/product-page/alpha-complete",
    notes: "Ganzjahres‑Hauptfutter (Balance aus Kondition & Vitalität)."
  },
  {
    id: "alpha_ice",
    name: "Alpha Ice 5mm",
    ...
    notes: "Spezialfutter für kaltes Wasser (6–12°C)."
  },
  {
    ...
    notes: "Konzentrat zum Beimischen (Farbkur ab ~16°C)."
  },
  {
    ...
    temp_min_c: 16,
    temp_max_c: 30,
    tags: ["Farbaufbau","Konditionierung"],
    url: "https://www.kodawari-koi.de/product-page/alpha-premium-bundle-complete-addon-color",
    notes: "Set aus Hauptfutter + Farb‑Addon (Mischung nach Bedarf)."
  }
];

// Entfernt alte/unerwünschte Futtersorten aus der lokalen DB (z.B. frühere Demo‑Defaults)
async function purgeFoods(){
  const killIds = new Set(["food_allround","food_color","food_wheatgerm","food_growth"]);
  const killNames = new Set(["allround","color","wheatgerm","growth","color wheatgerm"]);
  const foods = await getAll(state.db, "foods");
  for(const f of foods){
    const n = (f.name||"").trim().toLowerCase();
    if(killIds.has(f.id) || killNames.has(n)){
...
async function ensureDefaults(){
  ...
  // Entferne alte Demo-Sorten (Allround / Color / Wheatgerm / Growth)
  await purgeFoods();
...
async function loadSettings(){
  ...
  if(s && typeof s === "object"){
    ...
    // Passwortschutz entfernt – immer deaktiviert
...
function applyLockUI(){
  const lock = $("#lock");
  if(!lock) return; // Lock-Screen wurde entfernt
...
// Lock-Form existiert nicht mehr (legacy guard)
const _lockForm = $("#lockForm");
if(_lockForm){
  _lockForm.addEventListener("submit", async (e)=>{
    e.preventDefault();
